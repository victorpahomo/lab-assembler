...
result: .asciiz "El número menor es: "            # Mensaje para mostrar el número menor
...
main:
    ...
    bgt $t1, $t3, update_min  # Saltar a 'update_min' si $t1 es mayor que $t3
    move $t3, $t1     # Actualizar $t3 con $t1 si es menor
update_min:
    bgt $t2, $t3, print_result  # Saltar a 'print_result' si $t2 es mayor que $t3
    move $t3, $t2     # Actualizar $t3 con $t2 si es menor
    j print_result     # Saltar directamente a 'print_result' después de encontrar el mínimo
...
print_result:
    ...
    move $a0, $t3     # Cargar el valor de $t3 (el número menor) en $a0
    syscall
